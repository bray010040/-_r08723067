...
class Detect(object):
    ...
    def DaBaoSher(self,df):
        #1.2.前九根斜率很平
        #3.第10根上漲
        #4.第10根開始往上跳空
        #5.第10根要收高
        #6. 收高長度在前50根中PR值>=90
        cond1 = (df['trend9'].iloc[-2] < 0.5)
        cond2 = (df['trend9'].iloc[-2] > -0.5)
        cond3 = (df['direction'].iloc[-1] > 0)
        cond4 = (df['open'].iloc[-1] > df['high'].iloc[-2])
        cond5 = (df['close'].iloc[-1] > df['close'].iloc[-2])
        cond6 = (df['realbody_per'].iloc[-1] >= 90)
        if cond1 and cond2 and cond3 and cond4 and cond5 and cond6:
            return True
        else:
            return False
    def YaMeDay(self,df):
        ...
        cond2 = (df['trend9'].iloc[-2] > -0.5)
        cond3 = (df['direction'].iloc[-1] < 0)
        cond4 = (df['open'].iloc[-1] < df['low'].iloc[-2])
        cond5 = (df['close'].iloc[-1] < df['close'].iloc[-2])
        cond6 = (df['realbody_per'].iloc[-1] >= 90)
        if cond1 and cond2 and cond3 and cond4 and cond5 and cond6:
            return True
    ...
    def trend(self):
        ...
        #self.data['trend3'] = self.data['close'].rolling(3).apply(self.trending, raw=False)
    ...
    def signal(self):
        ...
        for idx in tqdm(self.data.index):
            ...
            if start_idx >= 0:
                ...
                if self.DaBaoSher(df):
                    self.data.loc[end_idx, 'DaBaoSher'] = 1
                elif self.YaMeDay(df):
                    self.data.loc[end_idx, 'YaMeDay'] = 1
    ...
    def result(self):
        ...
        print('DaBaoSher: {}'.format(self.data.loc[self.data['DaBaoSher'] == 1, 'DaBaoSher'].shape[0]))
        print('YaMeDay: {}'.format(self.data.loc[self.data['YaMeDay'] == 1, 'YaMeDay'].shape[0]))
