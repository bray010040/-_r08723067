...
class Detect(object):
    ...
    def BigReverseV(self,df):
        ...
        cond4 = (df['realbody_per'].iloc[-1] >= 90)
        cond5 = (df['close'].iloc[-1] > df['low'].iloc[-10])
        if cond1 and cond2 and cond3 and cond4 and cond5:
            return True
        else:
            return False
    def BigCrash(self,df):
        #1.前9根正斜率
        #2.第10根下跌
        #3.第10根開始向下跳空
        #4. 收低長度在前50根中PR值>=90
        ...
        cond1 = (df['trend9'].iloc[-2] > 0)
        cond2 = (df['direction'].iloc[-1] < 0)
        cond3 = (df['open'].iloc[-1] < df['low'].iloc[-2])
        cond4 = (df['realbody_per'].iloc[-1] >= 90)
        cond5 = (df['close'].iloc[-1] < df['low'].iloc[-10])
        if cond1 and cond2 and cond3 and cond4 and cond5:
            return True
    ...
    def process(self):
        ...
        self.data['BigReverseV'] = 0
        self.data['BigCrash'] = 0
        self.data['None'] = 0
    ...
    def signal(self):
        ...
        for idx in tqdm(self.data.index):
            ...
            if start_idx >= 0:
                ...
                if self.BigReverseV(df):
                    self.data.loc[end_idx, 'BigReverseV'] = 1
                elif self.BigCrash(df):
                    self.data.loc[end_idx, 'BigCrash'] = 1
